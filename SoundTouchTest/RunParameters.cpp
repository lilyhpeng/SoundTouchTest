...
RunParameters::RunParameters(const int nParams, const char * const paramStr[])
{
    ...
    fprintf(stderr, "Set [inFileName] = %s \n", inFileName);
    outFileName = (char*)paramStr[2];
    fprintf(stderr, "Set [outFileName] = %s \n", outFileName);
...
void RunParameters::parseSwitchParam(const string &str)
{
    ...
    switch (upS) 
    {
        case 't' :
            ...
            fprintf(stderr, "Set [tempo] = %f \n", tempoDelta);
            break;

        case 'p' :
            // switch '-pitch=xx'
            pitchDelta = parseSwitchValue(str);
            fprintf(stderr, "Set [pitch] = %f \n", pitchDelta);
            break;

        case 'r' :
            // switch '-rate=xx'
            rateDelta = parseSwitchValue(str);
            fprintf(stderr, "Set [rate] = %f \n", rateDelta);
            break;
